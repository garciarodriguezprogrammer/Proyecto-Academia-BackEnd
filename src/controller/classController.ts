...
export class ClassesController {
  ...
  async getClassById(req: Request, res: Response): Promise<void> {
    ...
    try {
      ...
      if (classDetail) {
        // Formatea la clase detallada para incluir nombres de estudiantes y más datos
        const formattedClassDetail = {
          ...classDetail,
          teacherName: classDetail.teacher?.user?.userName, // Suponiendo que los profesores también tienen usuarios asociados
          enrollments: classDetail.enrollments?.map((enrollment) => ({
            studentId: enrollment.student?.id,
            studentName: enrollment.student?.user?.userName, // Asegúrate que corresponde a tu modelo
          })),
        };
        res.json(formattedClassDetail);
      } else {
  ...
  async updateClass(req: Request, res: Response): Promise<void> {
    ...
    try {
      const classToUpdate = await classRepository.findOneBy({
        id: parseInt(id),
      });
      if (classToUpdate) {
